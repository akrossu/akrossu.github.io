...
<div class="max-w-content w-full">
    ...
    {#each Object.entries(images) as prop, index}
        <details open={index == 0}>
            ...
            <div class="grid grid-cols-2 gap-2 mb-8 photo-thumbnail">
                <div class="[&>*]:mb-2 last:[&>*]:mb-0 [&>*]:w-full">
                    {#each prop[1].left as src}
                        <Button on:click={() => {active = !active; modalImage = prop[1].title + "/" + src}}>
                            <img src="https://raw.githubusercontent.com/akrossu/akrossu.github.io/projectA/static/assets/imageboard/{prop[1].title}/thumbnails/{src}.webp" alt="" loading="lazy">
                        </Button>
                    {/each}
                </div>
                <div class="[&>*]:mb-2 last:[&>*]:mb-0 [&>*]:w-full">
    ...
    <div class:active={active} class="fixed hidden z-10 w-full h-full mica overflow-auto left-0 top-0 overflow-y-hidden">
        ...
        <button class="absolute top-0 left-0 w-full h-full" aria-label="close image" onclick={() => {active = !active; modalImage = null}}>
            <span class="absolute right-0 top-0 mr-4 p-4">X</span>
        </button>
        <img class="m-auto relative block max-w-[80%] max-h-screen photo-full" src="https://raw.githubusercontent.com/akrossu/akrossu.github.io/projectA/static/assets/imageboard/{modalImage}.JPG" alt="" loading="lazy">
        {/if}
    </div>
    <div class="my-32"></div>
</div>
